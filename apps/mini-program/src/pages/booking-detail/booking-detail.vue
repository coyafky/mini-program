...
export default {
  ...
  methods: {
    statusText(status) {
      const map = { pending: '待处理', contacted: '已联系', confirmed: '已确认', completed: '已完成', closed: '已关闭' };
      return map[status] || status || '待处理';
    },

    timeSlotText(slot) {
      const slots = { morning: '上午 09:00-12:00', afternoon: '下午 12:00-17:00', evening: '晚间 17:00-20:00' };
    ...
    async loadDetail() {
      try {
        const res = await this.$api.getReservationDetail(this.id);
